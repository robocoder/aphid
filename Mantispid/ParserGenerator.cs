...
namespace Mantispid
{
    public class ParserGenerator
    {
        ...
        private CodeStatementCollection GenerateLocals()
        {
            var statements = _locals.Select(x => CodeHelper.VarDecl(x.Value, x.Key)).ToArray();

            return new CodeStatementCollection(statements);
        }

        private CodeStatementCollection GenerateParseRuleBody(AphidExpression node)
        {
            switch (node.Type)
            {
                case AphidNodeType.BinaryOperatorExpression:
        ...
        private CodeStatementCollection GenerateMatchStatement(AphidExpression node)
        {
            switch (node.Type)
            {
                case AphidNodeType.IdentifierExpression:
        ...
        private CodeExpression GeneratePeek(AphidExpression node, out string varName)
        {
            switch (node.Type)
            {
                case AphidNodeType.IdentifierExpression:
